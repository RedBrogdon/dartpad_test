...
/// Object capable of testing values to see if they meet one or more criteria,
/// such as being of a specific type, having a property with a particular value,
/// etc.
abstract class AsyncMatcher {
  ...
  /// Builds a textual description of a particular failure to match. This should
...
class MatcherToAsyncMatcherAdapter extends AsyncMatcher {
  ...
  /// Default constructor. If a literal value ("ABCD", 1234, etc.) is given as
  /// the argument, it will be wrapped in a [Matcher] via [wrapMatcher], then
  /// wrapped again by this class.
  ...
  /// Returns the description generated by the [Matcher]'s describe method with
  /// no modification.
  @override
  Description describe(Description description) =>
...
/// Creates a CompletionMatcher. This is included for parity with API found in
/// the `test` package.
...
/// resulting value with another matcher. If the future generates an error, the
/// match automatically fails.
class CompletionMatcher extends AsyncMatcher {
  AsyncMatcher _innerMatcher;
  ...
  Future<MatchResult> matchAsync(dynamic item) async {
    ...
    if (item == null) {
      ...
      try {
        dynamic innerItem = await (item as Future);
